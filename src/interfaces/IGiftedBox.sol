...
interface IGiftedBox {
  function getGiftingRecord(uint256 tokenId) external view returns (GiftingRecord memory);

  function tokenAccountAddress(uint256 tokenId) external view returns (address);
