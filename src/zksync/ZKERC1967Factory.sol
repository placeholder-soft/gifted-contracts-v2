...
import {ZKERC1967Proxy} from "./ZKERC1967Proxy.sol";
import {ZKUpgradeableBeacon} from "./ZKUpgradeableBeacon.sol";
import {ZKERC1967BeaconProxy} from "./ZKERC1967BeaconProxy.sol";
...
contract ZKERC1967Factory {
    ...
    constructor() payable {
        ...
        address proxyAddress = address(new ZKERC1967Proxy{salt: proxySalt}());

        proxyHash = _extcodehash(proxyAddress);
        beaconHash = _extcodehash(address(new ZKUpgradeableBeacon()));
        beaconProxyHash = _extcodehash(address(new ZKERC1967BeaconProxy()));

        if (_predictDeterministicAddressZKsync(proxyHash, proxySalt) == proxyAddress) {
          emit log_string("Using ZKsync CREATE2 prediction");
            _useZKsyncCreate2Prediction = true;
        } else {
            _initCodeHashes[proxyHash] = keccak256(type(ZKERC1967Proxy).creationCode);
            _initCodeHashes[beaconHash] = keccak256(type(ZKUpgradeableBeacon).creationCode);
            _initCodeHashes[beaconProxyHash] = keccak256(type(ZKERC1967BeaconProxy).creationCode);
        }
    }
    ...
    function _deploy(
        ...
        if (codeType == 0) {
            instance = address(
                useSalt ? new ZKERC1967Proxy{salt: _validateSalt(salt)}() : new ZKERC1967Proxy()
            );
            /// @solidity memory-safe-assembly
            assembly {
                sstore(instance, admin)
            ...
            instance = address(
                useSalt
                    ? new ZKUpgradeableBeacon{salt: _validateSalt(salt)}()
                    : new ZKUpgradeableBeacon()
            );
            /// @solidity memory-safe-assembly
            assembly {
                sstore(instance, admin)
            ...
            instance = address(
                useSalt
                    ? new ZKERC1967BeaconProxy{salt: _validateSalt(salt)}()
                    : new ZKERC1967BeaconProxy()
            );
