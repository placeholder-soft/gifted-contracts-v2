...
contract GiftedBoxTest is Test, TestEvents {
  ...
  function testTokenBoundAccountCallDirectly() public {
    ...
    bytes32 salt = giftedBox.calculateSalt(tokenId);
  ...
  function test_TokenAccountAddress() public view {
    uint256 tokenId = 1;

    // Calculate expected salt
    bytes32 expectedSalt = giftedBox.calculateSalt(tokenId);
    // Get account address
    address account = registry.account(
      expectedSalt
    );
  ...
  function test_DifferentTokensHaveDifferentAccounts() public view {
    uint256 tokenId1 = 1;
    uint256 tokenId2 = 2;
  ...
  function test_AccountAddressesAreConsistent() public view {
    uint256 tokenId = 1;
  ...
  function test_BytecodeHashFromArtifact() public view {
    string memory artifact = vm.readFile("zkout/GiftedAccount.sol/GiftedAccount.json");
